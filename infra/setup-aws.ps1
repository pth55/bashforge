...
# ── 1. ECR Repositories ───────────────────────────────────────────
Write-Host "`n[1/9] Creating ECR repositories..."
foreach ($repo in @("frontend","backend","sandbox")) {
    $exists = aws ecr describe-repositories --repository-names "$PREFIX-$repo" --region $REGION 2>$null
...
Write-Host "`n[2/9] Creating ECS task execution role..."
$EXEC_ROLE = "bashforge-ecs-execution-role"
$execRoleExists = aws iam get-role --role-name $EXEC_ROLE 2>$null
if ($LASTEXITCODE -ne 0) {
    $assumePolicy = '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"ecs-tasks.amazonaws.com"},"Action":"sts:AssumeRole"}]}'
    $assumePolicy | Out-File -FilePath "$env:TEMP\assume-ecs.json" -Encoding utf8
    aws iam create-role --role-name $EXEC_ROLE --assume-role-policy-document file://$env:TEMP\assume-ecs.json | Out-Null
...
# ── 3. EC2 Instance Role ──────────────────────────────────────────
Write-Host "`n[3/9] Creating EC2 instance role..."
$EC2_ROLE = "bashforge-ec2-role"
$ec2RoleExists = aws iam get-role --role-name $EC2_ROLE 2>$null
if ($LASTEXITCODE -ne 0) {
    $assumeEC2 = '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"ec2.amazonaws.com"},"Action":"sts:AssumeRole"}]}'
    $assumeEC2 | Out-File -FilePath "$env:TEMP\assume-ec2.json" -Encoding utf8
    aws iam create-role --role-name $EC2_ROLE --assume-role-policy-document file://$env:TEMP\assume-ec2.json | Out-Null

    $ec2Policy = "{`"Version`":`"2012-10-17`",`"Statement`":[{`"Effect`":`"Allow`",`"Action`":[`"ecs:RunTask`",`"ecs:StopTask`",`"ecs:DescribeTasks`",`"ecs:ListTasks`",`"ecs:DescribeTaskDefinition`"],`"Resource`":`"*`"},{`"Effect`":`"Allow`",`"Action`":`"iam:PassRole`",`"Resource`":`"$EXEC_ROLE_ARN`"},{`"Effect`":`"Allow`",`"Action`":[`"ecr:GetAuthorizationToken`",`"ecr:BatchGetImage`",`"ecr:GetDownloadUrlForLayer`"],`"Resource`":`"*`"}]}"
...
Write-Host "`n[4/9] Creating EC2 instance profile..."
$EC2_PROFILE = "bashforge-ec2-profile"
$profileExists = aws iam get-instance-profile --instance-profile-name $EC2_PROFILE 2>$null
if ($LASTEXITCODE -ne 0) {
    aws iam create-instance-profile --instance-profile-name $EC2_PROFILE | Out-Null
...
# ── 5. ECS Cluster ────────────────────────────────────────────────
Write-Host "`n[5/9] Creating ECS cluster..."
...
Write-Host "`n[6/9] Creating CloudWatch log group..."
aws logs create-log-group --log-group-name "/ecs/bashforge-sandbox" --region $REGION 2>$null
...
Write-Host "`n[7/9] Fetching default VPC public subnets..."
...
# ── 8. Security Group ─────────────────────────────────────────────
Write-Host "`n[8/9] Creating sandbox security group..."
$SG_NAME = "bashforge-sandbox-sg"
$EXISTING_SG = (aws ec2 describe-security-groups --filters "Name=group-name,Values=$SG_NAME" "Name=vpc-id,Values=$DEFAULT_VPC" --query "SecurityGroups[0].GroupId" --output text --region $REGION 2>$null)
if (-not $EXISTING_SG -or $EXISTING_SG -eq "None") {
    ...
    aws ec2 authorize-security-group-ingress --group-id $SG_ID --protocol tcp --port 8765 --cidr 0.0.0.0/0 --region $REGION | Out-Null
...
Write-Host "`n[9/9] Registering ECS task definition..."
$SANDBOX_IMAGE = "$ECR_REGISTRY/$PREFIX-sandbox:latest"
$taskDef = "{`"family`":`"bashforge-sandbox`",`"networkMode`":`"awsvpc`",`"requiresCompatibilities`":[`"FARGATE`"],`"cpu`":`"256`",`"memory`":`"512`",`"executionRoleArn`":`"$EXEC_ROLE_ARN`",`"containerDefinitions`":[{`"name`":`"bash-session`",`"image`":`"$SANDBOX_IMAGE`",`"essential`":true,`"portMappings`":[{`"containerPort`":8765,`"protocol`":`"tcp`"}],`"environment`":[{`"name`":`"PORT`",`"value`":`"8765`"}],`"logConfiguration`":{`"logDriver`":`"awslogs`",`"options`":{`"awslogs-group`":`"/ecs/bashforge-sandbox`",`"awslogs-region`":`"$REGION`",`"awslogs-stream-prefix`":`"ecs`"}},`"readonlyRootFilesystem`":false,`"linuxParameters`":{`"capabilities`":{`"drop`":[`"ALL`"]},`"initProcessEnabled`":true}}]}"
...
Write-Host "`n[+] Looking up EC2 instance with key pair 'bashforge-cicd'..."
...
if ($INSTANCE_ID -and $INSTANCE_ID -ne "None") {
    ...
    $alreadyAttached = (aws ec2 describe-iam-instance-profile-associations --filters "Name=instance-id,Values=$INSTANCE_ID" --query "IamInstanceProfileAssociations[0].AssociationId" --output text --region $REGION 2>$null)
    if ($alreadyAttached -and $alreadyAttached -ne "None") {
        Write-Host "  Instance profile already attached (association: $alreadyAttached)"
    } else {
        aws ec2 associate-iam-instance-profile --instance-id $INSTANCE_ID --iam-instance-profile Name=$EC2_PROFILE --region $REGION | Out-Null
    ...
    Write-Host "  No running EC2 found with key pair 'bashforge-cicd' — attach manually after launch"
...
Write-Host ""
Write-Host " GitHub Actions secrets to add:"
